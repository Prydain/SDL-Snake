...
static void debugDrawLives(SDL_Surface* target, GameState* game, Renderer* rend)
{
	...
	{
		// Note: better draw these in pixel positions rather than tile positions
...
static void createL1(Renderer* rend)
{
	...
	rend->extra.animation = createRPGAnimation("data/char.png", 150);
	rend->extra.ani1UP = createSingleFrameAnimation("data/1up.png");
	rend->extra.aniHeart = createSingleFrameAnimation("data/heartContainer.png");
	rend->extra.aniStar = createSingleFrameAnimation("data/star.png");
	rend->extra.aniLive = createSingleFrameAnimation("data/live.png");
	rend->extra.aniBack = createSingleFrameAnimation("data/background.png");

	freeTileMap(&rend->tileMap);
	rend->tileMap.tileMapSurface = IMG_Load("data/tileset.png");
	...
	SDL_Surface* wallSurface = IMG_Load("data/boo.png");
...
void render(SDL_Surface* target, GameState* game, Renderer* rend)
{
	...
	if (game->gameMode == modeGameOver)
	{
		...
		SDL_Color color = TO_COLOR(CONST_COLOR_SCORE_TEXT);
		SDL_Surface* scoreSurface = TTF_RenderText_Solid(rend->font, scoreText, color);
		sprintf(scoreText, "Press any key to start a new game...");
...
Renderer* createRenderer(RendererType type)
{
	...
	result->font = TTF_OpenFont("data/font.ttf", 46);
	if (result->font == NULL)
	{
